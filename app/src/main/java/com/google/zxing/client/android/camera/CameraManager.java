...
public final class CameraManager {
    ...
    public synchronized Rect getFramingRectInPreview() {
        if (framingRectInPreview == null) {
            Rect framingRect = viewfinderView.getScanBoxRect();
            if (framingRect == null) {
                return null;
            }
            ...
            boolean landscape=!(cameraResolution.x>cameraResolution.y)^(screenResolution.x>screenResolution.y);
            int width = (int)((landscape?cameraResolution.x:cameraResolution.y)*scaleW);
            int height = (int)((landscape?cameraResolution.y:cameraResolution.x)*scaleH);
    ...
    public PlanarYUVLuminanceSource buildLuminanceSource(byte[] data, int width, int height) {
        ...
        boolean port=(cameraResolution.x>cameraResolution.y)^(screenResolution.x>screenResolution.y);
        if (port) {
            byte[] ver_data = new byte[data.length];
            for (int i = 0; i < width; i++) {
                for (int j = 0; j < height; j++) {
                    ver_data[height * i + height - 1 - j] = data[j * width + i];
                }
            }
            data=ver_data;
