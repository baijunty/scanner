...
final public class  BeepManager implements MediaPlayer.OnErrorListener, Closeable {
  ...
  synchronized public void updatePrefs() {
    ...
    vibrate = prefs.getBoolean(PreferencesKeys.KEY_VIBRATE, false);
...
//      context.setVolumeControlStream(AudioManager.STREAM_MUSIC);
  ...
  private static boolean shouldBeep(SharedPreferences prefs, Context activity) {
    boolean shouldPlayBeep = prefs.getBoolean(PreferencesKeys.KEY_PLAY_BEEP, true);
    if (shouldPlayBeep) {
  ...
  public synchronized boolean onError(MediaPlayer mp, int what, int extra) {
    if (what == MediaPlayer.MEDIA_ERROR_SERVER_DIED) {
      ...
      scannerView.displayError(scannerView.getContext().getString(R.string.msg_camera_framework_bug));
    } else {
